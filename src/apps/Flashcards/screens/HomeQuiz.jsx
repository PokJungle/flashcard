...
export default function HomeQuiz({ profile, onStartQuiz, onManageQuestions }) {
        ...
        <div className="flex gap-2 mb-4">
          {[
            ...
            <div key={m.label} className="flex-1 rounded-2xl py-2.5 px-2 text-center"
              ...
              <p className="text-base font-bold leading-none mb-1.5" style={{ color: m.color }}>{m.pts}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-3">
          <div className="px-4 py-3 border-b border-gray-50 flex items-center justify-between">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full"
              style={{ background: activeCount > 0 ? '#EEEDFE' : '#f3f4f6', color: activeCount > 0 ? '#534AB7' : '#9ca3af' }}>
                  ...
                  <div key={themeId}>
                    <div className="px-4 py-1.5 flex items-center gap-2"
                      ...
                      <span className="text-xs font-semibold uppercase tracking-wide"
                        style={{ color: color }}>{theme?.label}</span>
                    </div>
                    {themeDecks.map(deck => (
                      <SourceRow
                        key={deck.id}
...
function SourceRow({ label, checked, onToggle, action, color = '#534AB7' }) {
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 cursor-pointer border-t border-gray-50"
      onClick={onToggle}>
      ...
      <span className="flex-1 text-sm truncate"
        style={{ color: checked ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)', fontWeight: checked ? 500 : 400 }}>
        {label}
      </span>
      {action}
    </div>
