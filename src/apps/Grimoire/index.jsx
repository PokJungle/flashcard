...
// Traduction FR → EN pour la recherche
...
async function translateRecipe(recipe) {
  ...
  // Découper en morceaux de ~400 chars
  const chunks = []
  let current = ''
  for (const part of parts) {
    const candidate = current ? current + SEP + part : part
    if (candidate.length > 400 && current) {
      chunks.push(current)
      current = part
    } else {
      current = candidate
  ...
  for (const chunk of chunks) {
    ...
    translated.push(...result.split('||SEP||').map(s => s.trim()))
    await new Promise(r => setTimeout(r, 200)) // éviter le rate limiting
  }
...
async function searchRecipes(query, filters = {}) {
  ...
  const params = new URLSearchParams({
    ...
    query,
    number: 12,
    // On ne charge PAS addRecipeInformation ici → économise des points
  })
...
export default function Grimoire({ profile }) {
  ...
  const searchSeason = async (forceQuery = null) => {
    ...
    if (!q) {
      q = seasonData.en[Math.floor(Math.random() * seasonData.en.length)]
    } else {
      q = await translateToEn(q)
    }
    const results = await searchRecipes(q, filters)

    // Traduire les titres par petits groupes (3 à la fois)
    if (results.length > 0) {
      const groupSize = 3
      for (let i = 0; i < results.length; i += groupSize) {
        ...
        // Extraire les traductions via les marqueurs [0], [1], [2]
        group.forEach((r, j) => {
          const regex = new RegExp(`\\[${j}\\]\\s*([^\\[|]+)`)
  ...
  const openRecipe = async (recipe) => {
    setLoading(true)
    // Si déjà sauvegardée → données déjà traduites
    const saved = savedRecipes.find(r => r.spoonacular_id === recipe.id || r.spoonacular_id === recipe.spoonacular_id)
    if (saved) { setSelectedRecipe({ ...saved, fromDB: true }); setLoading(false); return }
    // Charger depuis Spoonacular
    ...
    details.extendedIngredients = (details.extendedIngredients || []).map((ing, i) => ({
      ...ing, original: translated.ingredients[i] || ing.original
    }))
  ...
  const saveRecipe = async (recipe) => {
    ...
    const toSave = {
      ...
      ingredients: (recipe.extendedIngredients || []).map((ing, i) => ({
  ...
  const isSaved = (id) => savedRecipes.some(r => r.spoonacular_id === id)

  // ─── DÉTAIL RECETTE ───────────────────────────────────────────────────────
  if (selectedRecipe) {
    const saved = isSaved(selectedRecipe.id || selectedRecipe.spoonacular_id)
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto">
                ...
                <div className="flex flex-wrap gap-2 justify-center mb-5">
                  {SEASON_INGREDIENTS[season].fr.map((ing, i) => (
                    <button key={ing} onClick={() => {
                      const enTerm = SEASON_INGREDIENTS[season].en[i]
                      setQuery(enTerm)
                      searchSeason(enTerm)
                    }}
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      {recipe.readyInMinutes && (
                        <span className="text-xs text-gray-400 flex items-center gap-0.5">
                          <Clock size={10} /> {recipe.readyInMinutes}m
                        </span>
                      )}
        ...
        {tab === 'grimoire' && (
          <div className="px-4 py-4 max-w-lg mx-auto">
            <p className="text-sm text-gray-400 mb-4">{savedRecipes.length} recette{savedRecipes.length !== 1 ? 's' : ''} sauvegardée{savedRecipes.length !== 1 ? 's' : ''}</p>
                  ...
                  <div key={day} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    {planned ? (
                      <button onClick={() => openRecipe(planned.recipe)}
                        ...
                        <button onClick={e => { e.stopPropagation(); removeFromPlan(day) }}
                          className="p-1.5 text-gray-300 hover:text-red-400 transition-colors">
                          <X size={15} />
                        </button>
                      </button>
                    ) : (
                      <button onClick={() => setTab('grimoire')}
                        className="w-full flex items-center gap-3 p-3 text-left opacity-50">
